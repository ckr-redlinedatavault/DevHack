...
export default function OrganizerRegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const res = await fetch("/api/organizer/auth/register", {
                method: "POST",
            ...
            if (res.ok) {
                // Instantly navigate to onboarding form to create event
            ...
            <div className="w-full max-w-[400px] space-y-10 relative">
                ...
                <div className="flex flex-col items-center space-y-4">
                    ...
                    <div className="text-center space-y-2">
                        <h1 className="text-3xl font-bold tracking-tight text-white font-outfit">Create account</h1>
                        <p className="text-zinc-500 text-sm font-medium">Join as a Hackathon Administrator</p>
                    </div>
                </div>
                ...
                <div className="bg-[#121214] border border-white/5 rounded-[2rem] p-8 shadow-2xl shadow-black/50">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-5">
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-600 group-focus-within:text-rose-500 transition-colors" />
                                    <Input
                                        name="name"
                        ...
                        >
                            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Create Dashboard Account"}
                        </Button>
                    </form>
                </div>
                ...
                <div className="text-center mt-8">
                    <p className="text-sm text-zinc-500 font-medium">
                        Already an admin?{" "}
                        <Link href="/organizer/login" className="text-rose-400 hover:text-rose-300 font-bold transition-colors">
                            Sign in
                        </Link>
                    </p>
                </div>
